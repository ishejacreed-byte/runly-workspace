...
const Profile = () => {
  const { user, logout, roleMode, updateUser } = useContext(AuthContext);
  ...
  const [editForm, setEditForm] = useState({ name: user?.name || '', bio: '', location: '' });
  ...
  const handleUpdateUsername = async () => {
    ...
    try {
      setSavingName(true);
      const res = await axios.put('http://localhost:5000/api/profile/update-username', { name: trimmed });
      updateUser(res.data);
      // If viewing own profile, refresh profile data to reflect change
      if (isMyProfile) fetchProfile();
  ...
  // Submit a review (keeps existing behavior)
  const submitReview = async () => {
    ...
    try {
      ...
      // The backend route in your repo expects POST /api/review/:id or similar.
      // Keep the existing call pattern used earlier in this filebase:
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfile(res.data);
      setEditForm({ name: res.data.name || '', bio: res.data.bio || '', location: res.data.location || '' });
if (isMyProfile && res.data.name) setNewName(res.data.name);
      // Keep local username state in sync when viewing own profile
      if (isMyProfile && res.data.name) setNewName(res.data.name);
    } catch (err) {
  ...
  // Profile picture upload handler (Cloudinary widget)
  const handleUpload = () => {
  ...
  setIsSaving(true);
  try {
    // 1) If name changed, update username first (this validates uniqueness)
    if (isMyProfile && editForm.name && editForm.name.trim() !== user?.name) {
      try {
  const resName = await axios.put('http://localhost:5000/api/profile/update-username', { name: editForm.name.trim() });
  if (resName?.data) updateUser(resName.data);
} catch (err) {
  const status = err?.response?.status;
  const data = err?.response?.data;
  if (status === 403 && data?.message) {
    alert(data.message); // or show a nicer UI toast/modal
  } else if (status === 409) {
  ...
  return;
    ...
    // 2) Update bio/location (existing route)
    await axios.put('http://localhost:5000/api/profile/update', { bio: editForm.bio, location: editForm.location });
    setShowEdit(false);
    // Refresh profile to reflect all changes
    await fetchProfile();
  } catch (err) {
    ...
    <div className="min-h-screen bg-background pb-24 p-4 space-y-6 animate-fadeSlideIn">
      ...
      <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm text-center relative overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-2 mt-2">
          ...
          <span className="text-xs text-muted-foreground font-medium">({profile.review_count})</span>
        </div>
      ...
      {/* FINANCIAL & PAYMENT SECTIONS (only my profile) */}
      {isMyProfile && (
        <>
          {roleMode === 'helper' ? (
            <div className="space-y-4 animate-in fade-in duration-500">
              ...
              <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-[2rem] shadow-sm relative overflow-hidden">
                ...
                <h4 className="text-4xl font-black text-emerald-600 mb-4">€{formatBudget(profile.total_earned)}</h4>
      ...
      {/* LEAVE A REVIEW (only when visiting someone else) */}
      {!isMyProfile && (
      ...
      {/* ACCOUNT ACTIONS (only my profile) */}
      {isMyProfile && (
  ...
  <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-card w-full max-w-sm rounded-[2rem] p-6 border border-border shadow-2xl">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowEdit(false)} className="p-2 hover:bg-secondary rounded-full" aria-label="Close edit modal">
          <X />
        </button>
      </div>
